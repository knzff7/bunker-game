...
async function generateNarrative(survivors, exiled, catastrophe) {
  ...
  const prompt = `Ты — циничный хроникёр конца света. Твоя задача — написать финальный отчёт о судьбе группы выживших в постапокалиптической игре "Бункер". Это не героическая история — это холодный разбор того, кто выжил, кто погиб и почему.

КАТАСТРОФА: ${catastrophe?.title || 'Неизвестная катастрофа'}
${catastrophe?.description || ''}
Срок изоляции в бункере: ${catastrophe?.duration || 'неизвестно'}

ВЫЖИВШИЕ — те кто попал в бункер:
${survivorsText}

ИЗГНАННЫЕ — те кого выбросили умирать:
${exiledText}

Напиши ПОДРОБНЫЙ финальный текст строго из 5 абзацев. Каждый абзац — минимум 4-5 предложений.

Абзац 1: Опиши момент закрытия бункера. Атмосфера, звуки, ощущения. Упомяни катастрофу снаружи.
Абзац 2: Разбери состав выживших по именам — кто полезен (профессии, навыки), кто балласт (фобии, болезни, бесполезный багаж). Будь конкретен и жесток в оценках.
Абзац 3: Опиши судьбу каждого изгнанного по имени — что их ждёт снаружи с их конкретными характеристиками. Детально и безжалостно.
Абзац 4: Предскажи конкретные проблемы которые возникнут в бункере — конфликты между конкретными людьми, нехватка навыков, болезни, психологические срывы из-за фобий.
Абзац 5: Финальная горькая оценка — выживут ли они на самом деле? Стоило ли это того?

Стиль: холодный, саркастичный, без сентиментальности. Называй игроков по именам. Упоминай конкретные характеристики.
Язык: русский. БЕЗ markdown, БЕЗ заголовков, только чистый текст абзацами разделёнными пустой строкой.`;
