...
async function generateNarrative(survivors, exiled, catastrophe) {
  ...
  const prompt = `Действуй как профессиональный сценарист и геймдизайнер настольных игр. Тебе необходимо написать захватывающий, атмосферный и глубокий финальный нарративный текст (эпилог) для игры «Бункер».

ВВОДНЫЕ ДАННЫЕ:

Тип катастрофы: ${catastrophe?.title || 'Неизвестная катастрофа'}
...
1. Анализ выживания: На основе характеристик оставшихся игроков (профессии, навыки, здоровье) опиши, как именно сложилась их жизнь в бункере через год. Используй конкретику: кто чинил системы, кто следил за психологическим климатом, а чьи фобии или болезни стали обузой.

2. Статистика и ресурсы: Подведи итог эффективности группы. Оцени в художественной форме процент вероятности возрождения цивилизации этой группой. Хватило ли им знаний для самообеспечения?

3. Призраки прошлого: Обязательно выдели анализ изгнанных игроков. Укажи, в чём именно была ошибка группы при их исключении. Например: «Вы выгнали биолога из-за его вредной привычки, но именно его знаний не хватило, когда в гидропонике завёлся грибок». Покажи, как навыки изгнанных могли бы закрыть критические бреши в выживании.

4. Итоговый вердикт: Сформулируй финальную оценку действий команды. Было ли решение оставить именно этих людей оптимальным или группа обречена из-за предвзятости при отборе?

СТРУКТУРА ОТВЕТА (строго соблюдай):
— Название финала (придумай на основе катастрофы и состава группы)
— Хроника выживания (подробный нарратив о жизни внутри)
— Упущенные возможности (анализ полезности изгнанных)
— Эпилог (финальная фраза о будущем этой колонии)

Тон: мрачный, реалистичный, с элементами драмы, логика основана на игровых характеристиках.
Язык: русский.
Называй всех игроков по именам, упоминай их конкретные характеристики.
НЕ используй markdown символы (* # ** и т.д.). Используй только обычный текст, заголовки разделов пиши КАПСЛОКОМ на отдельной строке, абзацы разделяй пустой строкой.`;
